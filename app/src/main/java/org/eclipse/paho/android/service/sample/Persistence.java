...
public class Persistence extends SQLiteOpenHelper implements BaseColumns {
  ...
  //subscription flags
    /** Table column for message subscription flag **/
    public static final String COLUMN_SUBSCRIBE_MESSAGE = "subscribeMessage";
    /** Table column for alarm subscription flag **/
    public static final String COLUMN_SUBSCRIBE_ALARM = "subscribeAlarm";
    /** Table column for status subscription flag **/
    public static final String COLUMN_SUBSCRIBE_STATUS = "subscribeStatus";
  ...
  private static final String SQL_CREATE_ENTRIES =
      ...
      "CREATE TABLE " + TABLE_CONNECTIONS + " (" +
          ...
          COLUMN_SUBSCRIBE_MESSAGE + INT_TYPE + COMMA_SEP +
          COLUMN_SUBSCRIBE_ALARM + INT_TYPE + COMMA_SEP +
          COLUMN_SUBSCRIBE_STATUS + INT_TYPE + COMMA_SEP +
  ...
  public void persistConnection(Connection connection) throws PersistenceException {
    ...
    values.put(COLUMN_device_Name, connection.getDeviceName());
    ...
    values.put(COLUMN_SUBSCRIBE_MESSAGE, connection.isMessageSubscribed());
    values.put(COLUMN_SUBSCRIBE_MESSAGE, connection.isAlarmSubscribed());
    values.put(COLUMN_SUBSCRIBE_MESSAGE, connection.isStatusSubscribed());
  ...
  public List<Connection> restoreConnections(Context context) throws PersistenceException
  {
    ...
    String[] connectionColumns = {
        ...
        COLUMN_RETAINED,
        COLUMN_QOS,
        COLUMN_SUBSCRIBE_MESSAGE,
        COLUMN_SUBSCRIBE_ALARM,
        COLUMN_SUBSCRIBE_STATUS,
        _ID

    };

    //how to sort the data being returned
    String sort = COLUMN_device_Name;
    ...
    for (int i = 0; i < c.getCount(); i++) {
      ...
      String deviceName = c.getString(c.getColumnIndexOrThrow(COLUMN_device_Name));
      ...
      boolean subscribeMessage = c.getInt(c.getColumnIndexOrThrow(COLUMN_SUBSCRIBE_MESSAGE)) == 1 ? true : false;
      boolean subscribeAlarm = c.getInt(c.getColumnIndexOrThrow(COLUMN_SUBSCRIBE_ALARM)) == 1 ? true : false;
      boolean subscribeStatus = c.getInt(c.getColumnIndexOrThrow(COLUMN_SUBSCRIBE_STATUS)) == 1 ? true : false;
      ...
      connection = Connection.createConnection(clientID, host, port, context, ssl, deviceName, subscribeMessage, subscribeAlarm, subscribeStatus);
